...
#     build_UTKFace_classification_dataset,
#     build_FairFace_classification_dataset
# )



def build_classification_dataset(split,attribute):
	"""
	...
	data = torch.load(BASE_PATH + '{}_FFHQ_{}_64x64.pt'.format(split,attribute))
	labels = torch.load(BASE_PATH + '{}_FFHQ_{}_labels_64x64.pt'.format(split,attribute))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, help='where to save outputs', default="./results")
    # parser.add_argument('--dataset', type=str, help='interested dataset: celeba, UTKFace, or FairFace', default="celeba")
    # parser.add_argument('--out_dir', type=str, help='where to save outputs',default="./results/celeba/attr_clf")
    parser.add_argument('--attribute', type=str, help='Senstive attribute', default="Blackhair")
    parser.add_argument('--clf_type', type=str, default='ResNet18', 
                        help='attr-clf type [default: ResNet18]')
    ...
    # if args.dataset == 'celeba':
        # if not args.multi:
    train_dataset = build_classification_dataset('train',args.attribute)
    valid_dataset = build_classification_dataset('val',args.attribute)
    test_dataset = build_classification_dataset('test',args.attribute)
    n_classes = 2
    #     else:
    ...
    #     train_dataset = build_UTKFace_classification_dataset('train')
    #     valid_dataset = build_UTKFace_classification_dataset('val')
    #     test_dataset = build_UTKFace_classification_dataset('test')
    #     n_classes = 2
        
    # elif args.dataset == 'FairFace':
    #     train_dataset = build_FairFace_classification_dataset('train')
    #     valid_dataset = build_FairFace_classification_dataset('val')
    #     test_dataset = build_FairFace_classification_dataset('test')
    #     n_classes = 2
    ...
    # build model
    model_cls = build_model(args.clf_type)
    if args.clf_type == 'ResNet18':
        model = model_cls(block=BasicBlock, layers=[2, 2, 2, 2], num_classes=n_classes, grayscale=False)
    elif args.clf_type == 'CNN5':
        model = model_cls(nc=3, ncf=64, num_classes=n_classes)
