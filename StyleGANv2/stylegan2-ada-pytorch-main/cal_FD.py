...
from metrics.FD.clf_models import ResNet18, BasicBlock
...
@click.option('--attribute', help='SA attribute in question', type=str, default="Gender")


def calc_metrics(ctx, network_pkl, data,metrics, mirror, gpus, verbose,attribute):
    ...
    Cweights=torch.load(os.path.join("../metrics/FD/results/{}".format(attribute),"model_best.pth.tar"))['state_dict']
    ...
    for i in tqdm(range(int(batchSize))):
        ...
        logits,probas=classifier(trans(image).float()/255)
        hardLabelsArray.append(np.argmax(probas.cpu().detach(),axis=1))
        
    hardLabelsArray=torch.cat(hardLabelsArray)
    label0=np.where(hardLabelsArray==0)[0]
    label1=np.where(hardLabelsArray==1)[0]
    dist=np.array([len(label0)/len(hardLabelsArray),len(label1)/len(hardLabelsArray)])
    FD=np.sqrt(np.sum((np.subtract(np.array([0.5,0.5]),dist))**2))
    
    print ("With {} samples, FD of {} is {}".format(imagesLen,attribute,FD))
