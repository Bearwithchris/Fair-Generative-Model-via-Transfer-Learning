...
def GAN_training_function(G, D_Fair, D_Quality, GD_Fair, GD_Quality, z_, y_, ema, state_dict, config):
    discriminator_loss, generator_loss = select_loss(config)

    def train(x, y, ratio):
        G.optim.zero_grad()
        D_Fair.optim.zero_grad()
        ...
        #Edit: Disable automatic toggling to avoid disrupting freezing
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                #Added horizontal flio
                D_fake, D_real = GD_Fair(z_[:config['batch_size']], y_[:config['batch_size']], transform(x[counter]), y_counter, train_G=False,
                    split_D=config['split_D'])
                ...
                # D_loss = (1-config['lambda']) * (D_loss_real + D_loss_fake) / \
                #     float(config['num_D_accumulations'])
                D_loss = (config['lambda']) * (D_loss_real + D_loss_fake) / \
                    float(config['num_D_accumulations'])

                D_loss.backward()
                counter += 1

            # Optionally apply ortho reg in D01001502
        ...
        for accumulation_index in range(config['num_G_accumulations']):
            ...
            DF_fake = GD_Fair(z_, y_, train_G=True, split_D=config['split_D'])
            DQ_fake = GD_Quality(z_, y_, train_G=True, split_D=config['split_D'])
            
            # we don't need to do anything for the generator loss
            G_loss = ( config['lambda']* generator_loss(DF_fake) + (1-config['lambda'])* generator_loss(DQ_fake) ) / float(config['num_G_accumulations'])
            G_loss.backward()
...
def save_and_sample(G, D_Quality, D_Fair, G_ema, z_, y_, fixed_z, fixed_y,
                    state_dict, config, experiment_name):
    utils.save_weights(G, D_Quality, D_Fair, state_dict, config['weights_root'],
    ...
    image_filename = '%s/%s/fixed_samples%d.jpg' % (config['samples_root'],
                                                    experiment_name,
                                                    state_dict['itr'])
    np_image_filename = '%s/%s/np_fixed_samples%d.npz' % (config['samples_root'],
                                                experiment_name,
                                                state_dict['itr'])
    #Save Fixed images                                           
    np.savez(np_image_filename,x=fixed_Gz.float().cpu().numpy())
...
''' This function runs the inception metrics code, checks if the results
    ...
    improvement. '''
def update_FID(G, D_Quality, D_Fair, G_ema, state_dict, config, FID, experiment_name, test_log, epoch):
    if not config['multi']:
