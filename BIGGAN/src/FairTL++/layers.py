...
class GBlock_AdaFM(nn.Module):
    def __init__(self, in_channels, out_channels,
                 which_conv=nn.Conv2d, which_bn=bn, activation=None,
                 upsample=None, fhidden=None):
        super(GBlock_AdaFM, self).__init__()
        ...
        if fhidden is None:
            self.fhidden = min(self.in_channels, self.out_channels)
        else:
            self.fhidden = fhidden
    
        # Conv layers
        self.conv1 = self.which_conv(self.in_channels, self.out_channels)
        self.small_adafm_1 = AdaFM(self.fhidden, self.in_channels) #Added
        self.conv2 = self.which_conv(self.out_channels, self.out_channels)
        self.small_adafm_2 = AdaFM(self.out_channels, self.fhidden) #Added
    ...
    def forward(self, x, y):
        ...
        #Edit: Activate ADAFM
        F_weights1=self.small_adafm_1(self.conv1.weight)
        #h = self.conv1(h)
        h=F_conv(h,F_weights1,bias=self.conv1.bias,stride=1,padding=1)
        h = self.activation(self.bn2(h, y))
        
        #Edit: Activate ADAFM
        F_weights2=self.small_adafm_2(self.conv2.weight)
        # h = self.conv2(h)
        h=F_conv(h,F_weights2,bias=self.conv2.bias,stride=1,padding=1)
...
class DBlock_AdaFM(nn.Module):
    def __init__(self, in_channels, out_channels, which_conv=SNConv2d, wide=True,
                 preactivation=False, activation=None, downsample=None,):
        super(DBlock_AdaFM, self).__init__()
        self.in_channels, self.out_channels = in_channels, out_channels
        ...
        self.small_adafm_1 = AdaFM(self.hidden_channels, self.in_channels) #Added
        self.conv2 = self.which_conv(self.hidden_channels, self.out_channels)
        self.small_adafm_2 = AdaFM(self.out_channels, self.hidden_channels) #Added
