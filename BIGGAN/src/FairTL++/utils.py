...
def prepare_parser():
    ...
    parser.add_argument(
        '--dataset', type=str, default='CA64TEST', #default='I128_hdf5',
    ...
    parser.add_argument('--start_eval', type=int, default=40, help='number of epochs after the model begins evaluation.')
    ...
    parser.add_argument('--perc', type=float, default='0.25', help="Flag for percentage of balanced dataset: [0.1, 0.25, 0.5, 1.0]")
    ...
    ### Hyper parameters for transfer Learning ####
    parser.add_argument(
        '--lambda', type=float, default=0.5,
        help='Weightage between D_fair and D_quality')
    ...
    parser.add_argument(
        '--num_save_copies', type=int, default=1,
        help='How many copies to save (default: %(default)s)')
    parser.add_argument(
        '--num_best_copies', type=int, default=50,
    ...
    parser.add_argument(
        '--test_every', type=int, default=1000,
    ...
    parser.add_argument(
        ...
        '(default: %(default)s)')
    parser.add_argument(
        '--name_suffix', type=str, default='celeba_90_10_perc0.25_impweight',
    ...
    parser.add_argument(
        '--ema_start', type=int, default=1000,
    ...
    parser.add_argument(
        '--resumePaused', action='store_true', default=False,
        help='Resume training for Paused TFl? (default: %(default)s)')
...
dset_dict = {'I32': dset.ImageFolder, 'I64': dset.ImageFolder,
             ...
             'STL10': dset.STL10, 'C10U': dset.CIFAR10Unsupervised, 'CelebA': dset.CelebA, 'CA64': dset.CelebA, 'CA64TEST': dset.CelebA}
imsize_dict = {'I32': 32, 'I32_hdf5': 32,
               'I64': 64, 'I64_hdf5': 64,
               'I128': 128, 'I128_hdf5': 128,
               'I256': 256, 'I256_hdf5': 256,
               'C10': 32, 'C100': 32, 'CINIC10': 32, 'STL10': 64, 'C10U': 32, 'CelebA': 32, 'CA64': 64, 'CA64TEST': 64, 'UTKFace': 64}
root_dict = {'I32': 'ImageNet', 'I32_hdf5': 'ILSVRC32.hdf5',
             'I64': 'ImageNet', 'I64_hdf5': 'ILSVRC64.hdf5',
             'I128': 'ImageNet', 'I128_hdf5': 'ILSVRC128.hdf5',
             'I256': 'ImageNet', 'I256_hdf5': 'ILSVRC256.hdf5',
             'C10': 'cifar', 'C100': 'cifar', 'CINIC10': 'cinic10/train', 'STL10': 'stl10', 'C10U': 'cifar', 'CelebA': 'celeba', 'CA64': 'celeba', 'CA64TEST': 'celeba', 'UTKFace':'UTKFace'}
...
classes_per_sheet_dict = {'I32': 50, 'I32_hdf5': 50,
                          ...
                          'C10': 10, 'C100': 100, 'CINIC10': 10, 'STL10': 10, 'C10U': 1, 'CelebA': 1, 'CA64': 1, 'UTKFace': 1}
...
def get_data_loaders(config, dataset, data_root=None, augment=False, 
    ...
    assert dataset == 'CA64' or dataset == 'CA64TEST' or dataset == 'UTKFace' and not augment
    ...
    if 'hdf5' in dataset:
        ...
        if augment:
            ...
            if dataset in ['C10', 'C100', 'CINIC10', 'STL10', 'C10U']:
                ...
                train_transform = [transforms.CenterCrop(140),
                                   transforms.Resize(image_size)]
            elif dataset in ['CA64TEST']:  # celeba-test
                # load balanced data
                print("Utilising Test data******************************************************************************************************")
                balanced_path = DATA_PATH + 'test_celeba_64x64.pt'
                # balanced_path = DATA_PATH + 'celeba_{}/celeba_balanced_train_data.pt'.format("90_10_perc0.25")
                balanced_labels_path = DATA_PATH + 'test_labels_celeba_64x64.pt'
                # normalize data to be [-1, 1]
                balanced_x = torch.load(balanced_path).float() / 127.5 - 1
                # balanced_y = torch.load(balanced_labels_path).long()
                balanced_y = torch.ones(len(balanced_x)).long()
                ...
                density_ratio = torch.ones_like(balanced_density_ratio).float()
                ...
                #Edited: (11/04/22) Train with unbalance data
                elif config['dummy'] == 1: # Only unbalanced
                    print ("-------------Only unbalanced data being used---------------------------")
                    x = unbalanced_x
                    y = unbalanced_y
                    ratios = unbalanced_density_ratio
                #Edited: (11/04/22) Train with balance data
                elif config['dummy'] == 2: # Only weighted
                    print ("-------------Only balanced data being used---------------------------")
                    x = balanced_x
                    y = balanced_y
                ...
                train_set = torch.utils.data.TensorDataset(x, y, ratios)
                 ...
                 # load balanced data
                 balanced_path = DATA_PATH + 'UTKFace_{}/UTKFace_balanced_train_data.pt'.format(bias_split)
                 balanced_labels_path = DATA_PATH + 'UTKFace_{}/UTKFace_balanced_train_labels.pt'.format(bias_split)
                 # normalize data to be [-1, 1]
                 balanced_x = torch.load(balanced_path).float() / 127.5 - 1
                 balanced_y = torch.load(balanced_labels_path).long()
                 # density ratio automatically set to all 1's
                 ...
                 # load unbalanced data
                 unbalanced_path = DATA_PATH + 'UTKFace_{}/UTKFace_unbalanced_train_data.pt'.format(bias_split)
                 unbalanced_labels_path = DATA_PATH + 'UTKFace_{}/UTKFace_unbalanced_train_labels.pt'.format(bias_split)
                 # normalize data to be [-1, 1]
                 unbalanced_x = torch.load(unbalanced_path).float() / 127.5 - 1
                 unbalanced_y = torch.load(unbalanced_labels_path).long()
                 ...
                 if config['reweight'] == 1:
                     print('using reweighting flattening coefficient of {}'.format(config['alpha']))
                     ratio_path = DATA_PATH + 'UTKFace_{}/UTKFace_unbalanced_train_density_ratios.pt'.format(bias_split)
                     unbalanced_density_ratio = torch.load(ratio_path)
                 else:
                     print('running baseline; all density ratios will be set to 1 (no reweighting will take place)!')
                     unbalanced_density_ratio = torch.ones_like(unbalanced_y).float()
...
def save_weights(G, D_Quality, D_Fair, state_dict, weights_root, experiment_name,
    ...
    #Save weights of D_Fairness
    torch.save(D_Fair.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_Fair', name_suffix])))
    torch.save(D_Fair.optim.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_Fair_optim', name_suffix])))
    
    #save weights of D_Quality
    torch.save(D_Quality.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_Quality', name_suffix])))
    torch.save(D_Quality.optim.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_Quality_optim', name_suffix])))
...
# Load a model's weights, optimizer, and the state_dict (Resume from mid transfer)
def load_weights_resume(G, D_Quality , D_Fair, state_dict, weights_root, experiment_name,
                 name_suffix=None, G_ema=None, strict=True, load_optim=True):
    root = '/'.join([weights_root, experiment_name])
    if name_suffix:
        print('Loading %s weights from %s...' % (name_suffix, root))
    ...
    if D_Quality is not None:
        D_Quality.load_state_dict(
            torch.load('%s/%s.pth' %
                       (root, join_strings('_', ['D_Quality', name_suffix]))),
            strict=strict)
        if load_optim:
            D_Quality.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['D_Quality_optim', name_suffix]))))
    if D_Fair is not None:
        D_Fair.load_state_dict(
            torch.load('%s/%s.pth' %
                       (root, join_strings('_', ['D_Fair', name_suffix]))),
            strict=strict)
        if load_optim:
            D_Fair.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['D_Fair_optim', name_suffix]))))
    # Load state dict
    for item in state_dict:
        state_dict[item] = torch.load(
            '%s/%s.pth' % (root, join_strings('_', ['state_dict', name_suffix])))[item]
...
def load_weights_AdaFM(G, D, state_dict, weights_root, experiment_name,
                 name_suffix=None, G_ema=None, strict=True, load_optim=True):
    root = '/'.join([weights_root, experiment_name])
    if name_suffix:
        print('Loading %s weights from %s...' % (name_suffix, root))
        ...
        print('Loading weights from %s...' % root)
    if G is not None:
        dict_G=torch.load('%s/%s.pth' %(root, join_strings('_', ['G', name_suffix])))
        #Edit: to load the dict of known parameters
        G=model_equal_part(G,dict_G)
        # G.load_state_dict(torch.load('%s/%s.pth' %(root, join_strings('_', ['G', name_suffix]))),strict=strict)
        if load_optim:
            G.optim.load_state_dict(torch.load('%s/%s.pth' % (root, join_strings('_', ['G_optim', name_suffix]))))
    if D is not None:
        dict_D=torch.load('%s/%s.pth' %(root, join_strings('_', ['D', name_suffix])))
        D=model_equal_part(D,dict_D)
        # D.load_state_dict(torch.load('%s/%s.pth' %(root, join_strings('_', ['D', name_suffix]))),strict=strict)
        if load_optim:
            D.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['D_optim', name_suffix]))))
    # Load state dict
    ...
    if G_ema is not None:
        dict_G_ema=torch.load('%s/%s.pth' %(root, join_strings('_', ['G_ema', name_suffix])))
        G_ema=model_equal_part(G_ema,dict_G_ema)
