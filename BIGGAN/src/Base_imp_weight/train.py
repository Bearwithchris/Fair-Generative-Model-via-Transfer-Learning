...
def run(config):
    ...
    state_dict = {'itr': 0, 'epoch': 0, 'save_num': 0, 'save_best_num_fair': 0, 'save_best_num_fid': 0,
                  'best_IS': 0, 'best_FID': 999999, 'best_fair_d': 999999, 'config': config}
    ...
    loaders = utils.get_data_loaders(
        config, **{**config, 'batch_size': D_batch_size,
    ...
    fixed_bs=512
    fixed_z, fixed_y = utils.prepare_z_y(fixed_bs, G.dim_z,
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        ...
        if (epoch >= config['start_eval']):
            # save generator at the end of each epoch
            utils.save_weights_G(G, state_dict, config['weights_root'],
                                   experiment_name, 'epoch%d' % state_dict['epoch'],
                                   G_ema if config['ema'] else None)
